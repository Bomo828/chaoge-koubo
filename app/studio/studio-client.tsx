...
import { compileViralCaptionCues, viralCaptionTemplateContract } from "../../lib/viral-caption-contract";
...
function normalizeViralCaptionsForReview(captions: ViralCaption[]) {
  // The transcription worker owns only the recognized text and timing. Keep
...
export function Video({ memberId, busy, action, onPointsChange, viralImportAsset }: { memberId: string; busy: boolean; action: () => void; onPointsChange: (points: number) => void; viralImportAsset?: { id: string; name: string; mediaUrl: string; importUrl?: string; contentType?: string; viralWorkflow?: ViralWorkflowManifest | null } | null }) {
  ...
  function confirmViralTranscriptReview() {
    ...
    const captionContract = viralCaptionTemplateContract(viralTemplate);
    const reviewedCaptions = viralReviewCaptions.map((caption) => {
      const text = caption.text.trim();
      const layout = planViralCaptionLayout(text, caption.captionLines, captionContract.lineMaxUnits);
    ...
    // Manual edits use the same template-safe compiler as the AI director.
    // This can create additional consecutive cues inside the original time
    // range, but it never asks a second AI to reinterpret the user's wording.
    const captions = compileViralCaptionCues(reviewedCaptions, viralTemplate);
  ...
  async function retryViralCaptionPlan() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "AI内容导演请求失败，请稍后重试。");
      if (!data.planReady || !data.title || !Array.isArray(data.captions) || !data.captions.length) {
      ...
      setViralAnalysisSummary("AI内容导演已重新理解完整口播，并一次完成标题、语义分段、模板排版、提亮关键词与重点词规划。");
    } catch (error) {
      setViralCaptionPlanReady(false);
      setViralCaptionPlanError(error instanceof Error ? error.message : "AI内容导演请求失败，请稍后重试。");
    } finally {
    ...
    try {
      ...
      if (sourceCaptions.length) {
        try {
          ...
          if (planResponse.ok && Array.isArray(planData.captions) && planData.captions.length) {
      ...
      const archivedAssetId = stableAssetId(`lip-sync|${completedTaskId || data.videoUrl}`);
      window.sessionStorage.setItem("merchant-studio-ai-explainer-source", JSON.stringify({
        id: archivedAssetId,
        name: "对口型成片.mp4",
        mediaUrl: `/api/member/assets/${encodeURIComponent(archivedAssetId)}`,
        fallbackMediaUrl: data.videoUrl,
  ...
  function openLipSyncResultInViralEditor() {
    ...
    setViralCaptionPlanError(speechViralPlanReady
      ? ""
      : "已复用对口型字幕时间轴，但AI内容导演尚未完成。请在确认文案时点击“重新规划内容”。");
  ...
  function openLipSyncResultInSuperEditor() {
    if (!lipSyncResultUrl || lipSyncBusy) return;
    const existing = (() => {
      try {
        return JSON.parse(window.sessionStorage.getItem("merchant-studio-ai-explainer-source") || "null") as Record<string, unknown> | null;
      } catch {
        return null;
      }
    })();
    window.sessionStorage.setItem("merchant-studio-ai-explainer-source", JSON.stringify({
      ...(existing || {}),
      name: "对口型成片.mp4",
      fallbackMediaUrl: lipSyncResultUrl,
      transcript: script.trim(),
      title: speechViralTitle || viralTitleFromKnownScript(script),
      duration: Math.max(1, speechAudioDuration || speechCaptions.at(-1)?.end || 1),
      captions: speechCaptions.map((caption) => ({ ...caption })),
      source: "lip-sync",
      consumed: false,
    }));
    window.location.assign("/ai-explainer/index.html");
  ...
  async function extractViralTranscript() {
    ...
    try {
      ...
      // ASR ends after recognized text and timing. The single content director
      // then plans title, semantic groups and template-aware line breaks here;
      // the render worker only consumes and validates that confirmed result.
      ...
      if (!directVideoAi) {
        try {
          ...
          setViralCaptionPlanError(aiData.planReady ? "" : aiData.warning || "AI内容导演未完成，请在确认前重新规划内容。");
        } catch (error) {
          aiSummary = `${error instanceof Error ? error.message : "大模型校对暂时不可用"} 已保留真实语音识别结果，并按完整句整理。`;
          setViralCaptionPlanReady(false);
          setViralCaptionPlanError(error instanceof Error ? error.message : "AI内容导演未完成，请在确认前重试。");
        }
      }
  ...
  function drawViralFrame(
    ...
    if (currentCaption) {
      ...
      const captionLineLength = viralCaptionTemplateContract(viralTemplate).lineMaxUnits;
        ...
        <section className={`viral-source-pane viral-edit-step-card ${viralFiles.length ? "is-complete" : "is-active"}`}>
          ...
          <section className="viral-source-transcript">
            <header>
              ...
              <button type="button" disabled={!viralFiles.length || viralImportPreparing || viralTranscriptBusy || viralProcessBusy || Boolean(viralPendingTask)} onClick={() => viralCaptions.length ? openViralTranscriptReview() : void extractViralTranscript()}>{viralImportPreparing ? "正在读取会员视频…" : viralTranscriptBusy ? `AI 正在识别并规划 · ${viralTranscriptProgress}%` : viralCaptions.length ? "查看并修改文案" : "AI 识别并规划"}</button>
            </header>
            {viralCaptions.length ? <div className={`viral-transcript-status ${viralCaptionsConfirmed ? "is-confirmed" : ""}`}>
              <span><b>{viralCaptionsConfirmed ? "标题与字幕已确认" : "AI内容规划已完成，等待确认"}</b><small>{plainViralTitle(viralTitle) || `${viralCaptions.length} 段字幕`}</small></span>
        ...
        <section className="viral-transcript-dialog" role="dialog" aria-modal="true" aria-labelledby="viral-transcript-dialog-title">
          ...
          <div className="viral-transcript-dialog-body">
            {!viralCaptionPlanReady ? <div className="viral-dialog-ai-status" role="alert">
              <div><strong>当前不是完整的AI内容导演结果</strong><span>{viralCaptionPlanError || "当前只保留了真实语音时间轴，请重新规划标题、字幕和关键词。"}</span></div>
              <button type="button" disabled={viralCaptionPlanBusy} onClick={() => void retryViralCaptionPlan()}>{viralCaptionPlanBusy ? "AI内容导演规划中…" : "重新规划内容"}</button>
            </div> : <div className="viral-dialog-ai-status is-ready"><strong>AI内容导演已完成</strong><span>标题、语义分段、模板换行、提亮关键词和重点词已一次生成，可继续手动调整。</span></div>}
            ...
            <div className="viral-dialog-caption-list" aria-label="AI内容导演规划的字幕">
              ...
              {viralReviewCaptions.map((caption, index) => {
                ...
                <input
                  ...
                  title={keywordValid ? "可手动修改AI内容导演规划的提亮关键词" : "提亮关键词必须完整出现在当前字幕中"}
                  onChange={(event) => {
...
function AssetVideoThumbnail({ item }: { item: MemberAssetItem }) {
  ...
  const [ready, setReady] = useState(Boolean(item.coverUrl));
  ...
  if (item.coverUrl) {
    return <img src={item.coverUrl} alt="" loading="lazy" decoding="async" />;
  }

  return <>
    {!ready ? <span className={`asset-video-placeholder ${failed ? "is-error" : ""}`}>{failed ? "预览读取失败" : "正在读取首帧"}</span> : null}
    <video
      ref={videoRef}
...
export function Assets({
  ...
  onUseViral: (asset: MemberAssetItem) => void;
  onUseSuperEditor: (asset: MemberAssetItem) => void;
}) {
  ...
  return <>
    ...
    {error ? <div className="asset-state is-error"><b>资产空间暂时无法打开</b><span>{error}</span><button onClick={() => void loadAssets()}>重新加载</button></div> : loading ? <div className="asset-state"><i /><b>正在读取会员资产</b><span>生成记录正在按项目整理</span></div> : projects.length ? <div className="asset-project-list">{projects.map(([projectName, projectItems]) => <section className="asset-project" key={projectName}>
      <header><div><small>项目</small><h2>{projectName}</h2></div><span>{formatAssetTime(projectItems[0].createdAt)} · {projectItems.length} 个文件</span></header>
      <div className="asset-item-grid">{projectItems.map((item) => <article className={`asset-item is-${item.kind}`} key={item.id}>
        <button className="asset-media" onClick={() => { setViewerError(""); setSelectedAsset(item); }} aria-label={`打开${item.name}`}>
          {item.kind === "image" ? <img src={item.mediaUrl} alt={item.name} loading="lazy" decoding="async" /> : item.kind === "video" ? <AssetVideoThumbnail item={item} /> : <span>{item.kind === "voice" ? "声" : "音"}</span>}
          <i>{item.kind === "video" ? "▶" : item.kind === "image" ? "查看" : "播放"}</i>
        </button>
        <div><b>{item.name}</b><span>{formatAssetTime(item.createdAt)} · {formatAssetSize(item.sizeBytes)}</span></div>
        {item.kind === "video" ? <div className="asset-item-workflows"><button onClick={() => onUseViral(item)}>✦ 一键网感</button><button onClick={() => onUseSuperEditor(item)}>◆ 一键超级剪辑</button></div> : null}
        <div className="asset-item-actions"><a href={memberAssetDownloadUrl(item)} download={item.name}>下载</a><button onClick={() => void deleteAsset(item)}>删除</button></div>
      </article>)}</div>
    </section>)}</div> : <div className="asset-state is-empty"><b>还没有{filter === "all" ? "会员资产" : filter === "image" ? "生成图片" : filter === "video" ? "生成短视频" : "声音文件"}</b><span>完成一次 AI 生成后，结果会自动按项目名称与时间出现在这里。</span></div>}
    {selectedAsset ? <div className="asset-viewer-backdrop" role="dialog" aria-modal="true" aria-label="查看会员资产" onMouseDown={(event) => { if (event.currentTarget === event.target) setSelectedAsset(null); }}><section className="asset-viewer">
      <button className="asset-viewer-close" aria-label="关闭" onClick={() => setSelectedAsset(null)}>×</button>
      <header><small>{selectedAsset.projectName}</small><h2>{selectedAsset.name}</h2><span>{formatAssetTime(selectedAsset.createdAt)}</span></header>
      {selectedAsset.kind === "image" ? <img src={selectedAsset.mediaUrl} alt={selectedAsset.name} /> : selectedAsset.kind === "video" ? <video key={selectedAsset.mediaUrl} src={selectedAsset.mediaUrl} poster={selectedAsset.coverUrl || undefined} controls autoPlay playsInline preload="metadata" disablePictureInPicture onLoadedData={() => setViewerError("")} onError={() => setViewerError("视频暂时无法读取，请重试或重新同步资产。")}/> : <audio src={selectedAsset.mediaUrl} controls autoPlay />}
      {viewerError ? <div className="asset-viewer-error" role="alert">{viewerError}</div> : null}
      <footer><a href={memberAssetDownloadUrl(selectedAsset)} download={selectedAsset.name}>↓ 下载文件</a>{selectedAsset.kind === "video" ? <button className="asset-viewer-viral" onClick={() => onUseViral(selectedAsset)}>✦ 一键网感</button> : null}{selectedAsset.kind === "video" ? <button className="asset-viewer-super" onClick={() => onUseSuperEditor(selectedAsset)}>◆ 一键超级剪辑</button> : null}<button onClick={() => setSelectedAsset(null)}>关闭</button></footer>
    </section></div> : null}
  </>;
}
