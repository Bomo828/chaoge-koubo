...
4. 逐个说明素材的可视信息、适合表达的语义和关键词；素材 id 必须原样返回。
5. 没有转写时 transcript_status 为 visual_only；有转写时为 available。

只返回 JSON：
{"topic":"内容主题","summary":"一句话摘要","industry":"行业","audience":"受众","key_points":["要点1"],"cover_titles":["标题1","标题2","标题3"],"recommended_cover_styles":[6,10],"materials":[{"id":"素材id","meaning":"语义","keywords":["关键词"]}],"transcript_status":"visual_only"}`;
...
2. 每个素材只在真正有解释价值的时间出现，单次1.5到4秒。
3. mode 只能是 full、pip、strip。细节用 full；保留人物表达用 pip；人物和素材比较用 strip。
4. 素材视频只作为无声画面，不能改变口播音频和总时长。
5. 不得编造不存在的素材 ID，不得让镜头重叠。
6. 至少安排一个素材镜头；有多个素材时尽量都使用。

只返回 JSON：
{"summary":"一句话导演思路","shots":[{"asset_id":"素材id","start":3.2,"end":6.2,"mode":"pip","reason":"为什么此处出现"}]}`;
